...
def generate_clips(video: Video, db: Session) -> list[Clip]:
    ...
    for index, (start, end) in enumerate(segments, start=1):
        ...
        _cut_segment(source_path, start, end, clip_abs_path)
        _extract_thumbnail(source_path, (start + end) / 2, thumb_abs_path)
...
def regenerate_clip_file(video: Video, clip: Clip) -> None:
    ...
    _cut_segment(source_path, clip.start_time, clip.end_time, clip_abs_path)
    ...
    if clip.thumbnail_url:
        ...
        _extract_thumbnail(source_path, (clip.start_time + clip.end_time) / 2, thumb_abs_path)
...
def _vertical_fit_filter_complex(fps: int | None = None) -> str:
    """Build a filter graph that fits the *entire* source frame into the
    1080x1920 output canvas without cropping any of it.

    The source is scaled down (preserving aspect ratio) to fit fully inside
    the canvas, then centered over a blurred, cropped copy of itself that
    fills the remaining frame -- the standard letterboxed "reels/shorts"
    treatment for non-vertical source footage, so nothing from the original
    picture is ever trimmed.
    """
...
def _cut_segment(source_path: str, start: float, end: float, dest_path: str) -> None:
    ...
    source frame fitted (never cropped) via `_vertical_fit_filter_complex`.
    """
    cmd = [
        settings.FFMPEG_PATH,
        "-y",
        ...
        source_path,
        "-filter_complex",
        _vertical_fit_filter_complex(fps=CLIP_OUTPUT_FPS),
        "-map",
        "[outv]",
        "-map",
        "0:a?",
...
def _extract_thumbnail(source_path: str, timestamp: float, dest_path: str) -> None:
